...
{
    // Reset to defaults
    TextEditor::TextBlockUserData *userData = TextEditor::BaseTextDocumentLayout::testUserData(currentBlock());
    if (userData) {
    ...
    for (int i = 0; i < tokens.count(); ++i) {
        ...
        if (category >= 0)
            setFormat(tk.start(), tk.count(), m_formats[category]);
    }

    // Visualize white space, if enabled
    const QTextCharFormat whiteSpaceFormat = m_formats[Format_VisualWhitespace];
